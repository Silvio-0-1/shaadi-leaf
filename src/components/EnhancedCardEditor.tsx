...
import { Heart, Calendar, MapPin, Move } from "lucide-react";
...
const EnhancedCardEditor = ({
  ...
  const { elementFontSizes, updateFontSizeFromResize, setFontSize, getFontSize } = useDynamicFontSizing({
    brideName: 36,
    groomName: 36,
    weddingDate: 14,
    venue: 14,
  ...
  const handleElementMove = useCallback(
    (elementId: string, newPosition: ElementPosition) => {
      ...
      setPositions((prev) => {
        ...
        if (elementId.startsWith("photo-")) {
          newPositions = {
            ...prev,
            photos:
              prev.photos?.map((photo) => (photo.id === elementId ? { ...photo, position: finalPosition } : photo)) ||
              [],
          };
        } else {
          newPositions = {
            ...prev,
            [elementId]: elementId === "photo" ? { ...prev.photo, position: finalPosition } : finalPosition,
          };
        }
    ...
    (elementId: string, newSize: { width: number; height: number }) => {
      ...
      setPositions((prev) => {
        ...
        if (elementId.startsWith("photo-")) {
          newPositions = {
            ...prev,
            photos: prev.photos?.map((photo) => (photo.id === elementId ? { ...photo, size: newSize } : photo)) || [],
          };
  ...
  const handleTextChange = useCallback(
    (field: keyof WeddingCardData, value: string) => {
  ...
  const handleDoubleClick = useCallback(
    (elementId: string) => {
      console.log(
        "🟡 EnhancedCardEditor handleDoubleClick called for:",
        elementId,
        "current editingElement:",
        editingElement,
      );
  ...
  const reset = useCallback(() => {
    const defaultPos = template?.defaultPositions
      ? {
          ...
          photo: template.defaultPositions.photo || defaultPositions.photo,
          photos:
            cardData.uploadedImages && Array.isArray(cardData.uploadedImages) && cardData.uploadedImages.length > 1
      ...
      setPositions((prev) => {
        ...
        if (elementId.startsWith("photo-")) {
          newPositions = {
            ...prev,
            photos:
              prev.photos?.map((photo) => (photo.id === elementId ? { ...photo, position: newPosition } : photo)) || [],
          };
        } else {
          newPositions = {
            ...prev,
            [elementId]: elementId === "photo" ? { ...prev.photo, position: newPosition } : newPosition,
          };
        }
  ...
  const handleFontSizeChange = useCallback(
    (elementId: string, newSize: number) => {
  ...
  const handleFontFamilyChange = useCallback(
    (elementId: string, newFamily: string) => {
      // Update customization based on element type
      const fontKey =
        elementId === "brideName" || elementId === "groomName"
          ? "heading"
          : elementId === "weddingDate"
            ? "date"
            : elementId === "venue"
    ...
    if (toolbarRef && template) {
      ...
      toolbarRef.current = {
        ...
        fontSize: selectedElement
          ? elementFontSizes[selectedElement] ||
            (selectedElement === "brideName" || selectedElement === "groomName"
              ? 32
              : selectedElement === "weddingDate"
                ? 24
                : selectedElement === "venue"
                  ? 20
                  : selectedElement === "message"
                    ? 16
                    : 16)
          : undefined,
        fontFamily: selectedElement
          ? getFontFamilyForElement(
              selectedElement === "brideName" || selectedElement === "groomName"
                ? "heading"
                : selectedElement === "weddingDate"
                  ? "date"
    ...
    <div className="space-y-6">
      ...
      <Card
        ...
        onClick={(e) => {
          ...
          const isClickingOnDraggableElement =
            clickTarget.closest("[data-draggable-element]") ||
          ...
          const isClickingOnToolbar =
            ...
            (clickTarget.className &&
              clickTarget.className.includes &&
              (clickTarget.className.includes("select-") || clickTarget.className.includes("radix-")));
      ...
      >
        {/* Grid Overlay */}
        <GridOverlay visible={showGridlines} gridSize={gridSize} containerRef={cardRef} />

        {/* Decorative Elements */}
        {!template?.backgroundImage && (
          <>
            <div className="absolute top-0 left-0 right-0 h-1 z-0" style={{ backgroundColor: colors.primary }} />
            <div className="absolute bottom-0 left-0 right-0 h-1 z-0" style={{ backgroundColor: colors.primary }} />
          </>
        )}
        ...
        <div className="relative h-full flex items-center justify-center p-8">
          ...
          {cardData.uploadedImages && Array.isArray(cardData.uploadedImages) && cardData.uploadedImages.length > 0 && (
            <>
              {cardData.uploadedImages.length === 1 ? (
                ...
                >
                  <div
                    className={`w-full h-full shadow-xl transition-all duration-200 ${
                      cardData.customization?.photoShape === "circle"
                        ? "rounded-full"
                        : cardData.customization?.photoShape === "square"
                          ? "rounded-none"
                          : "rounded-xl"
                    }`}
                    style={{
                ...
                positions.photos?.map(
                  (photo, index) =>
                    cardData.uploadedImages &&
                    cardData.uploadedImages[index] && (
                      <AdvancedDraggableElement
                        key={photo.id}
                        id={photo.id}
                        position={photo.position}
          ...
          >
            <div className="w-full h-full flex items-center justify-center" data-draggable-element="brideName">
          ...
          >
            <div className="w-full h-full flex items-center justify-center" data-draggable-element="groomName">
          ...
          {cardData.venue && (
            ...
            >
              <div className="w-full h-full flex items-center justify-center" data-draggable-element="venue">
                {editingElement === "venue" ? (
                  ...
                  >
                    {(() => {
                      ...
                      return (
                        <MapPin
                          style={{ width: `${iconSize}px`, height: `${iconSize}px`, flexShrink: 0, opacity: 0.85 }}
                        />
                      );
                    })()}
                    <span
          ...
          {cardData.message && (
            ...
            >
              <div className="w-full h-full flex items-center justify-center" data-draggable-element="message">
