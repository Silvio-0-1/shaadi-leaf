...
const AdvancedDraggableElement = ({
  ...
  const getTextColorStyles = useCallback(() => {
    ...
    return colorKey && customization.textColors[colorKey] ? { color: customization.textColors[colorKey] } : {};
  }, [id, customization]);
  ...
  const optimizedInteraction = useCallback(
    (clientX: number, clientY: number) => {
      ...
      animationFrameRef.current = requestAnimationFrame(() => {
        ...
        if (isDragging && !isLocked) {
          ...
          if (isTextElement() && onFontSizeChange) {
            // Skip resizing for text elements - DynamicTextBox handles its own resizing
            return;
            ...
            if (handle) {
              switch (resizeDirection) {
                ...
                case "ne":
                  newWidth = Math.max(minSize.width, Math.min(maxSize.width, startSize.width + deltaX));
                  newHeight =
                    handle.proportional && maintainAspectRatio
          ...
          {resizable && isSelected && !isLocked && !isTextElement() && (
            <>
              {resizeHandles.map((handle) => (
                ...
                >
                  {handle.proportional && <div className="w-1.5 h-1.5 bg-white rounded-full" />}
                </div>
              ))}
            </>
          )}
